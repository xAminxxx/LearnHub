...
public class TrainerService {
    ...
    @Transactional
    public Trainer createTrainer(TrainerRequest req) {
        if (trainerRepository.existsByEmail(req.email())) {
            throw new IllegalArgumentException("Trainer email already in use: " + req.email());
        }
    ...
    @Transactional
    public Trainer updateTrainer(Long id, TrainerRequest req) {
    ...
    private void applyUpsert(Trainer trainer, TrainerRequest req) {
        ...
        Specialization spec = specializationRepository.findById(req.specializationId())
                .orElseThrow(() -> new ResponseStatusException(BAD_REQUEST,
                        "Invalid specializationId: " + req.specializationId()));
