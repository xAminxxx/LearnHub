...
public class TrainerRestController {
    ...
    @PostMapping
    public TrainerDto createTrainer(@RequestBody TrainerUpsertRequest req) {
        return DtoMapper.toDto(trainerService.createTrainer(req));
    }

    @PutMapping("/{id}")
    public TrainerDto updateTrainer(@PathVariable Long id, @RequestBody TrainerUpsertRequest req) {
